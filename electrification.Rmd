...
colnames(L84_2017) <- c("Year", "BBL", "BIN", "Catagory", "GSF", "Address", "Borough", "Built", "NGas (kBtu)", "Electric (kBtu)" , "1 Oil (kBth)", "2 Oil (kBth)", "4 Oil (kBth)", "6 Oil (kBth)","count")
...
GSF_bins <- L84_2017 %>%
  group_by(bins) %>%
  summarise( GSF = sum(GSF), Num.Buildings = sum(count))
