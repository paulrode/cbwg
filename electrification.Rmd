...
# Feature engineering to make consistent with base data frame. 
  ...
  colnames(L84_2015) <- c( "id", "Name", "BBL", "Zip", "Boruugh", "GSF", "Type", "Built", "No.Buildings", "Oil1", "Oil2", "Oil4", "Oil56", "Diesel", "Steam", "N.Gas", "Elect", "Water", "GHG", "Data" , "Address")
  
L84_2015$Type <- as.factor(L84_2015$Type)
  
levels(L84_2015$Type) <-list(
...
"Parking", "Refrigerated Warehouse", "Strip Mall", "Other - Entertainment/Public Assembly", "Other - Mall", "Museum", "Indoor Arena", "Social/Meeting Hall", "Police Station", "Supermarket/Grocery Store", "Wholesale Club/Supercenter", "Enclosed Mall", "Fitness Center/Health Club/Gym", "Movie Theater", "Library", "Other - Services", "Performing Arts", "Food Sales","Automobile Dealership", "Other - Recreation", "Restaurant", "Other - Public Services", "Bowling Alley", "Personal Services (Health/Beauty, Dry Cleaning, etc.)", "Repair Services (Vehicle, Shoe, Locksmith, etc.)", "Lieu de culte", "Manufacturing/Industrial Plant", "Worship Facility",  "Social/Meeting Hall", "Courthouse", "Usine de fabrication/industrie"), 
...
L84_2020$Type2 <- L84_2020$Type 
L84_2020$Type <- as.factor(L84_2020$Type)

levels(L84_2020$Type) <-list(
"Housing"=c("Multifamily Housing", "Senior Living Community", "Résidence pour personnes âgées", "Autre"),
"Office" =c("Office", "Other", "Mixed Use Property", "Financial Office", "Data Center", 
"Courthouse", "Entreposage libre-service"), 
"Hospital" =c("Senior Care Community", "Medical Office", "Hospital (General Medical & Surgical)" , "Residential Care Facility", "Urgent Care/Clinic/Other Outpatient" , "Outpatient Rehabilitation/Physical Therapy" , "Ambulatory Surgical Center" , "Other - Specialty Hospital"), 
"Retail" = c("Non-Refrigerated Warehouse", "Self-Storage Facility", "Retail Store", "Distribution Center", 
"Parking", "Refrigerated Warehouse", "Strip Mall", "Other - Entertainment/Public Assembly", "Other - Mall", "Museum", "Indoor Arena", "Social/Meeting Hall", "Police Station", "Supermarket/Grocery Store", "Wholesale Club/Supercenter", "Enclosed Mall", "Fitness Center/Health Club/Gym", "Movie Theater", "Library", "Other - Services", "Performing Arts", "	
Mailing Center/Post Office" , "Stadium (Open)", "	
Lifestyle Center", "	
Prison/Incarceration", "Food Service" , "Food Sales","Automobile Dealership", "Other - Recreation", "Restaurant", "Other - Public Services", "Wastewater Treatment Plant", "Transportation Terminal/Station", "Other - Restaurant/Bar"  ,"Bowling Alley", "Bank Branch", "	
Mailing Center/Post Office", "Bar/Nightclub" , "Personal Services (Health/Beauty, Dry Cleaning, etc.)", "Repair Services (Vehicle, Shoe, Locksmith, etc.)", "Lieu de culte", "Manufacturing/Industrial Plant", "Worship Facility","Fire Station" , "Social/Meeting Hall", "Courthouse", "Usine de fabrication/industrie"), 
"HigherEd" = c("Laboratory", "Other - Technology/Science" , "College/University", "Adult Education", "Vocational School"), 
"K-12" = c("K-12 School", "Other - Education", "Pre-school/Daycare"), 
"Hotel" = c("Hotel", "Other - Lodging/Residential", "Immeuble à logements multiples", "Residence Hall/Dormitory"))
...
SameStore <- SameStore %>% 
  ...
  relocate(EUI, .after = Address) 
SameStore$Data <- as.numeric(SameStore$Data)



# Get rid of outliers

SameStore_no_outliers <- SameStore %>% 
  filter(EUI < 500 & EUI > 5)  
...
Types_no_outliers <- SameStore_no_outliers %>%
  group_by(Type) %>%  
  summarise("Number" = sum(No.Buildings), "GSF" = sum(GSF), "T.Energy" = sum(T.Energy), SizeBin, "tCO2e_2024" = sum(tCO2e_2024))  %>% 
...
LL84Emissions_2019 <- sum(SameStore_no_outliers %>% filter(Data == 2019)%>% summarise(tCO2e_2024))
