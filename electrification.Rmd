...
BMEmission <- SameStore_no_outliers %>% 
  select(Type, Data, T.Energy, tCO2e_2024) %>%
  group_by(Data) %>% 
  filter(Type == "Housing") %>% 
  summarise(T.Energy_kBTU = median(T.Energy), Emissions_tCO2e = median(tCO2e_2024))

BMEmission %>% 
  mutate(T.Energy_kBTU = T.Energy_kBTU/BMEmission$T.Energy_kBTU[1]-1) %>% 
  mutate(Emissions_tCO2e = Emissions_tCO2e/BMEmission$Emissions_tCO2e[1]-1) %>%
  ggplot(aes(Data, T.Energy_kBTU)) +
  geom_line(col = "purple") +
  geom_line(aes(y = Emissions_tCO2e), col = "red")
