...
SameStore2 <- SameStore %>% 
  select(Name, BBL, Oil1, Oil2, Oil4, Oil56, Diesel, Steam, N.Gas, Elect) %>% 
  gather(key = "Energy", value = "Utilities", Oil1, Oil2, Oil4, Oil56, Diesel, Steam, N.Gas, Elect, -Name,     -BBL, factor_key = TRUE) %>% 
...
SameStore <- SameStore %>% 
  mutate("SizeBin" = cut(SameStore$GSF, breaks = c(0, 25000, 50000, 500000,10000000), include.lowest = TRUE, labels = c("Small", "MidSized", "Large", "Large"), right = TRUE)) %>% 
...
Types_no_outliers <- SameStore_no_outliers %>%
  ...
  select(Type, Number, GSF, T.Energy)
...
Types %>% 
  ...
  mutate(Type = fct_reorder(Type, GSF)) %>%
  kable()
