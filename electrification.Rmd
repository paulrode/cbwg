...
L84_2017 %>% 
  mutate( Electricity.Use...Grid.Purchase..kBtu. == Electricity.Use...Grid.Purchase..kBtu. + District.Chilled.Water.Use..kBtu.) -> L84_2017
...
Types <- unique(L84_2017_S_G$Primary.Property.Type...Self.Selected, incomparables = FALSE)



Types <- Types[5]
L84_2017_S_G %>% filter(Primary.Property.Type...Self.Selected %in% Types ) -> L84_2017_S_G


L84_2017_S_G %>% summarise("NGas.(kBtu)" = sum(Natural.Gas.Use..kBtu. , na.rm = TRUE ), "Oil.No1" = sum(Fuel.Oil..1.Use..kBtu., na.rm = TRUE), "Oil.No2" = sum(Fuel.Oil..2.Use..kBtu., na.rm = TRUE), "Oil,No4" = sum(Fuel.Oil..4.Use..kBtu., na.rm = TRUE), "Oil.No5&6" = sum(Fuel.Oil..5...6.Use..kBtu., na.rm = TRUE), "Diesel.(kBtu)" = sum(Diesel..2.Use..kBtu., na.rm = TRUE), "Propane.(kBtu)" = sum(Propane.Use..kBtu., na.rm = TRUE), "Steam.(kBtu)" = sum(District.Steam.Use..kBtu., na.rm = TRUE), "Elect.(kBtu)" = sum(Electricity.Use...Grid.Purchase..kBtu., na.rm = TRUE)) -> Total.Energy
