...
L84_2017 <- read_excel("data/2017_nyc_benchmarking.xlsx", sheet = "Information and Metrics", col_names = TRUE, na = ""  ) %>%   mutate(Year = 2017) %>% data.frame()  %>% select(`BBL...10.digits`, "gsf" = DOF.Gross.Floor.Area..ft².)

L84_2016 <- read_excel("data/2016_nyc_benchmarking.xlsx", sheet = "Information and Metrics", col_names = TRUE, na = ""  ) %>%   mutate(Year = 2016) %>%  data.frame() %>% select(`BBL...10.digits`,"gsf" = DOF.Gross.Floor.Area )
