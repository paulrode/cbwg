...
# Add carbon parameters dataframes 
...
Types_no_outliers <- SameStore_no_outliers %>%
  group_by(Type) %>%  
  summarise("Number" = sum(No.Buildings), "GSF" = sum(GSF), "T.Energy" = sum(T.Energy))  %>% 
  mutate(Number_no_out = Number, GSF_1000xSF_no_out  = GSF/1000, T.Energy_MMBTU_no_out = T.Energy/1000) %>% arrange(desc(T.Energy_MMBTU_no_out)) %>% 
  select(Type, Number_no_out, GSF_1000xSF_no_out, T.Energy_MMBTU_no_out) 

Outlier_error <- inner_join(Types, Types_no_outliers, by = "Type") %>% 
  mutate(N = (Number - Number_no_out)/Number, GSF = (GSF_1000xSF- GSF_1000xSF_no_out)/GSF_1000xSF, T.Energy = (T.Energy_MMbtu - T.Energy_MMBTU_no_out)/T.Energy_MMbtu ) %>% 
  select(Type, Number, Number_no_out, N, GSF_1000xSF, GSF_1000xSF_no_out, GSF,T.Energy_MMbtu, T.Energy_MMBTU_no_out, T.Energy)
...
I imported years 2015 through 2020 from the NYC OpenData. To gain consistancy I filtered out sites that are not reprensented in all years based on thier associated Borough-Bloack-Lot (BBL) codes.  `r length(L84_2015$BBL)`
...
#Basic stats on data files 
summary(L84_2015)


#Table of buildings in dataset 
Types %>% 
    select(Type, Number, GSF, T.Energy_MMbtu) %>% 
    mutate(across(where(is.numeric), signif, 3)) %>%  
    kable(format = "html", digits = 6, col.names = c("Use Type", "Quantity", "GSF", "Total Energy (MMBTU)" )) %>%     kable_styling(full_width = FALSE)
...
=======
  ...
  ggplot(aes(GSF, CUI)) +
  geom_point() +
  geom_hline(aes(yintercept = 0.00846, color = "blue")) +
  geom_hline(aes(yintercept = 0.00453, color = "red")) +
  labs(title = "Office Carbon by year by Scope", x = "Submission Yeaer", y = "tCO2e") %>% 
  ggsave( plot = last_plot(), device = "jpeg",
       path = "C:/Users/paulr/Documents/R/cbwg/", scale = 1 )
>>>>>>> 7c88861c1f108e9b2886d5cbfcb92ce7b5dd88e3
