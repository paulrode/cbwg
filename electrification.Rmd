...
L84_2017 %>%
  group_by(Primary.Property.Type...Self.Selected) %>%
  summarise(`Total.Thermal (kBTU)`= sum(`Total.Thermal (kBTU)`)) %>%
  arrange(desc(`Total.Thermal (kBTU)`), by_group = TRUE) %>%
  mutate("Total.Thermal %" = 100 * `Total.Thermal (kBTU)`/sum(`Total.Thermal (kBTU)`)) -> TT_Table
TT_Table$`Total.Thermal %` <- format(TT_Table$`Total.Thermal %`, digits = 1, scientific = FALSE)
