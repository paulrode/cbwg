...
L84_2015[10:21] <- apply(L84_2015[10:21], 2, as.numeric )
...
L84_2015 <- L84_2015 %>% mutate("T.Energy" = (Oil2 + Oil4 + Oil56 + Steam + N.Gas + Elect)) %>%
  ...
  mutate("Type" = fct_lump(Type, n=9, w = T.Energy))  %>% 
  arrange(id, T.Energy) 
...
L84_2016[c(7,10:21)] <- apply(L84_2016[c(7,10:21)], 2, as.numeric )
...
L84_2017[10:21] <- apply(L84_2017[10:21], 2, as.numeric )
...
L84_2018[10:21] <- apply(L84_2018[10:21], 2, as.numeric )
...
L84_2019[10:21] <- apply(L84_2019[10:21], 2, as.numeric )
...
L84_2020 <- L84_2020 %>% mutate("T.Energy" = (Oil2 + Oil4 + Oil56 + Steam + N.Gas + Elect)) %>%
  ...
  relocate(T.Energy, .after = Address) %>% 
  relocate(EUI, .after = T.Energy) %>% 
  mutate("Type" = fct_lump(Type, n=12, w = T.Energy))  %>% 
  arrange(id, T.Energy) 
