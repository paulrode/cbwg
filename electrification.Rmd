...
L84_2017 <- read_excel("data/2017_nyc_benchmarking.xlsx", sheet = "Information and Metrics", col_names = TRUE, na = ""  ) %>%   mutate(Year = 2017) %>%  
...
L84_2017$Year.Built <- as.character (L84_2017$Year.Built)

L84_2016 <- read_excel("data/2016_nyc_benchmarking.xlsx", sheet = "Information and Metrics", col_names = TRUE, na = ""  ) %>%   mutate(Year = 2016) %>%  
...
L84_2015 <- read_excel("data/2015_nyc_benchmarking.xlsx", sheet = "2015 Data Reported in 2016", col_names = TRUE, na = ""  ) %>%   mutate(Year = 2015) %>%  
  select("Year", "Property Name", "Borough" , "GSF" =  "Largest Property Use Type - Gross Floor Area (ft²)", "Year Built", "Fuel Oil #1 Use (kBtu)","Fuel Oil #2 Use (kBtu)" , "Fuel Oil #4 Use (kBtu)", "Fuel Oil #5 & 6 Use (kBtu)", "Diesel #2 Use (kBtu)", "District Steam Use (kBtu)", "Natural Gas Use (kBtu)", "Electricity Use - Grid Purchase (kBtu)", "Reported NYC Building Identification Numbers (BINs)") %>% data.frame()
